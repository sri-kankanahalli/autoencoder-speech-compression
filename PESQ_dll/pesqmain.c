...
void double_array_to_signal(const double* signal, int signal_size,
                      SIGNAL_INFO* siginfo) {
    // TODO: ERROR CHECKING
    siginfo->Nsamples = signal_size + 2 * SEARCHBUFFER * Downsample;

    siginfo->data = (float *)safe_malloc((siginfo->Nsamples + 
                                          DATAPADDING_MSECS * (Fs / 1000)) * sizeof(float) );

    float* copy_ptr = siginfo-> data;
    for (int i = SEARCHBUFFER * Downsample; i > 0; i--)
        *(copy_ptr++) = 0.0f;

    for (int i = 0; i < signal_size; i++)
        *(copy_ptr++) = (float)signal[i];

    for (int i = DATAPADDING_MSECS * (Fs / 1000); i > 0; i--)
        *(copy_ptr++) = 0.0f;

    for (int i = SEARCHBUFFER * Downsample; i > 0; i--)
        *(copy_ptr++) = 0.0f;

    siginfo->VAD = safe_malloc(siginfo-> Nsamples * sizeof(float) / Downsample);
    siginfo->logVAD = safe_malloc(siginfo-> Nsamples * sizeof(float) / Downsample);
}

double pesq(const void* ref, const void* deg, int ref_size, int deg_size) {
    SIGNAL_INFO ref_info;
    SIGNAL_INFO deg_info;
    ERROR_INFO err_info;

    const double* ref_double = (double*)ref;
    const double* deg_double = (double*)deg;

    long Error_Flag = 0;
    char* Error_Type = "Unknown error type.";

    double_array_to_signal(ref_double, ref_size, &ref_info);
    double_array_to_signal(deg_double, deg_size, &deg_info);
...
const double align_filter_dB[26][2] = {{0.,-500},
                                 {50., -500},
                                 {100., -500},
                                 {125., -500},
                                 {160., -500},
...
const long WB_InIIR_Nsos = 1L;
const float WB_InIIR_Hsos[LINIIR] = {
