...
void freq_warping (int number_of_hz_bands, float *hz_spectrum, float *pitch_pow_dens, long frame) {
...
#define    MAX_NUMBER_OF_BAD_INTERVALS        1000
    ...
    for (frame = 0; frame <= stop_frame; frame++) {
        ...
        freq_warping (Nf / 2, hz_spectrum_ref, pitch_pow_dens_ref, frame);

        peak = maximum_of (pitch_pow_dens_ref, 0, Nb);    

        freq_warping (Nf / 2, hz_spectrum_deg, pitch_pow_dens_deg, frame);
